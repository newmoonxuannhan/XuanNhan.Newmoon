...
class WCPA_MC
{
    ...
    public function get_con_unit($product, $to_currency = false, $from_currency = false,$toMultiplyWithShopPrice=false)
    {
        ...
        if ($this->conversion_unit === false) {
            ...
            if ($this->conversion_unit == 1) {
                $this->conversion_unit = floatval(apply_filters('wcml_raw_price_amount', 100)/100);
            }
            if ($this->conversion_unit == 1) {
    ...
    public function mayBeConvert($price, $to_currency = false, $from_currency = false)
