        ...
        foreach ($meta_data as $meta) {
            ...
            if (($matches = $this->check_wcpa_meta($data)) !== false) {
                if (isset($wcpa_meta_data[$matches[1]])) {
                    ...
                    if (isset($wcpa_meta_data_item['form_data']->hideFieldIn_order) && $wcpa_meta_data_item['form_data']->hideFieldIn_order) {
...
//                    $wcpa_meta_key_info[$data->id] = $wcpa_meta_data_item['type'];
    ...
    public function checkout_create_order_line_item($item, $cart_item_key, $values, $order)
    {
        ...
        foreach ($values[WCPA_CART_ITEM_KEY] as $v) {
            ...
            if ($conversion_unit !== 1 && isset($v['price']) && $v['price'] !== false) {
                if (is_array($v['price'])) {
                    $v['price'] = array_map(
